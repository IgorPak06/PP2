...
def phone_book_proccess():
    ...
    if variant == 2:
        print("1 - Add into phone book; 2 - Update; 3 - Query data; 4 - Delete from phonebook")
        variant = int(input("Write variant to write data into table: "))
        if variant == 1:
            insert_console()
        if variant == 2:
...
def delete_data():
    filter_by = input("Write first name, last name or phone number to delete (Pick only one): ")
    cur.execute("DELETE FROM resident_data WHERE firstname = %s OR lastname = %s OR phone_number = %s",
                (filter_by, filter_by, filter_by))
    conn.commit()
